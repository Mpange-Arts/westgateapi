        ...
        uploadCategory.single('img')(req, res, async function (err) {
            if (err instanceof multer.MulterError) {
                if (err.code === 'LIMIT_FILE_SIZE') {
                    err.message = 'File size is too large. Maximum filesize is 30MB.';
                }
            ...
            let imageUrl = 'no_url';
            if (req.file) {
                imageUrl = `http://192.168.40.209:3000/image/category/${req.file.filename}`;
            }
...
router.put('/:id', asyncHandler(async (req, res) => {
    try {
        ...
        uploadCategory.single('img')(req, res, async function (err) {
            if (err instanceof multer.MulterError) {
                if (err.code === 'LIMIT_FILE_SIZE') {
                    err.message = 'File size is too large. Maximum filesize is 20MB.';
                }
            ...
            if (req.file) {
                image = `http://192.168.40.209:3000/image/category/${req.file.filename}`;
            }
